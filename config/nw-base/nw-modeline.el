...
(defvar nw-modeline-active-window nil)

(defun nw-modeline-set-active-window (windows)
  (when (not (minibuffer-window-active-p (frame-selected-window)))
    (setq nw-modeline-active-window (selected-window))))

(add-function :before pre-redisplay-function #'nw-modeline-set-active-window)

(defun nw-modeline-active-p ()
  (eq nw-modeline-active-window (get-buffer-window)))

(defun nw-modeline-status ()
  (let* ((active (nw-modeline-active-p))
...
(setq-default
 ...
 '(
   ...
   (:eval (propertize (shorten-directory default-directory 10) 'face
                      (if (nw-modeline-active-p)
                          'mode-line-folder-face
                        'mode-line-folder-inactive-face)))
   (:eval (propertize (nw-buffer-filename) 'face
                      (if (nw-modeline-active-p)
